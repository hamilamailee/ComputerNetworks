...
struct pkt *last_recieved_A;
struct pkt *last_sent_A;
struct pkt *last_recieved_B;
struct pkt *last_sent_B;
...
struct pkt *cpy_packet(struct pkt p);
...
struct pkt *cpy_packet(struct pkt p)
{
  ...
  mypktptr->seqnum = p.seqnum;
  mypktptr->acknum = p.acknum;
  mypktptr->checksum = p.checksum;
  for (int i = 0; i < 20; i++)
    mypktptr->payload[i] = p.payload[i];
  return mypktptr;
}
...
int A_output(struct msg message)
{
  if (last_sent_A != NULL)
  {
    if (last_recieved_A == NULL)
      return -1;
    if (last_recieved_A->seqnum != last_sent_A->seqnum)
    {
      // printf("Last packet has not been acked yet\n");
      return -1;
    }
  }
  ...
  p->acknum = last_sent_A == NULL ? 0 : !last_sent_A->acknum;
  p->seqnum = last_sent_A == NULL ? 0 : last_sent_A->seqnum + 1;
  p->checksum = get_checksum(p);
  tolayer3(0, *p);
  starttimer(0, 15);
  last_sent_A = cpy_packet(*p);
  return 1;
}

int matching_data(struct msg message, struct pkt packet)
{
  for (size_t i = 0; i < 20; i++)
    if (message.data[i] != packet.payload[i])
      return 0;
  return 1;
}
...
{
  int waiting_ack = last_sent_A == NULL ? 0 : last_sent_A->acknum;
  if (waiting_ack == packet.acknum)
  {
    last_recieved_A = cpy_packet(packet);
    stoptimer(0);
  }
}
...
void A_init()
{
  last_recieved_A = malloc(1 * sizeof(struct pkt));
  last_sent_A = malloc(1 * sizeof(struct pkt));
  last_recieved_A = NULL;
  last_sent_A = NULL;
}
...
{
  int waiting_ack = last_recieved_B == NULL ? 0 : !last_recieved_B;
  ...
  if (packet.acknum != waiting_ack)
  {
    // printf("Wrong ack\n");
    return;
  }
  last_recieved_B = cpy_packet(packet);
  struct msg *message = malloc(1 * sizeof(struct msg));
...
void B_init()
{
  last_recieved_B = malloc(1 * sizeof(struct pkt));
  last_sent_B = malloc(1 * sizeof(struct pkt));
  last_recieved_B = NULL;
  last_sent_B = NULL;
}
...
struct pkt packet;
{
  ...
  evptr->pktptr = mypktptr;         /* save ptr to my copy of packet */
                                  ...
                                  n     currently in the medium on their way to the destination */
  lastime = simul_time;
  for (q = evlist; q != NULL; q = q->next)
